...
static rx8130ce_handle_t require_device(void)
{
    ...
    if (i2c_device_handle(RX8130CE_I2C_ADDR_DEFAULT, &dev) != ESP_OK) {
        STRRES_ERROR(STR_I2C_RX8130CE_ADDRESSING_FAILED, RX8130CE_I2C_ADDR_DEFAULT);
        return NULL;
    }
    ...
    if (!handle) {
        ...
        if (rx8130ce_create(&config, &handle, &report) != ESP_OK) {
            STRRES_ERROR(STR_I2C_RX8130CE_NO_RESPONSE, RX8130CE_I2C_ADDR_DEFAULT);
            return NULL;
        }
    } else {
...
static void print_time(const struct timeval *tv, const char *label)
{
    ...
    STRRES_PRINTF(STR_I2C_RX8130CE_TIME_LINE, label, text);
}
...
int cmd_rx8130ce_time(int argc, char **argv)
{
    ...
    if (err == ESP_ERR_RX8130CE_NOT_SET) {
         ...
         */
        STRRES_ERROR(STR_I2C_RX8130CE_NO_VALID_DATE);
        return -1;
    }
    if (err != ESP_OK) {
        STRRES_ERROR(STR_I2C_RX8130CE_READ_FAILED, esp_err_to_name(err));
        return -1;
    }
    ...
    if (now.tv_sec > 1000000000) {
        ...
        STRRES_PRINTF(STR_I2C_RX8130CE_DRIFT, drift);
    } else {
        STRRES_PRINTF(STR_I2C_RX8130CE_SYSTEM_UNSET);
    }

    if (rx8130ce_power_was_lost(rtc)) {
        STRRES_PRINTF(STR_I2C_RX8130CE_POWER_LOST);
    }
    return 0;
}
...
int cmd_rx8130ce_set(int argc, char **argv)
{
    ...
    gettimeofday(&now, NULL);
    if (now.tv_sec < 1000000000) {
        STRRES_ERROR(STR_I2C_RX8130CE_SYSTEM_CLOCK_UNSET);
        return -1;
    }
    ...
    if (err != ESP_OK) {
        STRRES_ERROR(STR_I2C_RX8130CE_SET_FAILED, esp_err_to_name(err));
        return -1;
    }

    print_time(&now, "RTC set:");
    STRRES_PRINTF(STR_I2C_RX8130CE_POWER_FLAG_CLEARED);
    return 0;
}
